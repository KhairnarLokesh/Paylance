...
import { Briefcase, LayoutDashboard, FolderOpen, MessageSquare, Wallet, Bell, LogOut, User, Search, Plus, Menu, X, UserCircle, } from "lucide-react";
...
export default function DashboardLayout({ children }) {
    ...
    return (<div className="flex h-screen bg-background overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 transform border-r border-border bg-sidebar transition-transform duration-200 lg:static lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-full flex-col">
          {/* Logo */}
          <div className="flex h-16 items-center justify-between border-b border-sidebar-border px-4">
            <div className="flex items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => setCurrentView('landing')}>
      ...
      <div className="flex flex-1 flex-col">
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border bg-background/80 px-4 backdrop-blur-md lg:px-6">
          ...
          <div className="flex items-center gap-2">
            {user?.role === "client" && (<Button size="sm" onClick={() => setCurrentView("create-project")}>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="relative">
                  ...
                  {unreadCount > 0 && (<span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-destructive text-xs text-destructive-foreground">
                      {unreadCount}
                    </span>)}
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-80">
                ...
                {notifications.length === 0 ? (<div className="p-4 text-center text-sm text-muted-foreground">
                    ...
                    {notifications.slice(0, 5).map((notif) => (<DropdownMenuItem key={notif._id || notif.id} className="flex flex-col items-start p-3">
