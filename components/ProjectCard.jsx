...
const ProjectCard = ({ project, onClick }) => {
    ...
    const getStatusStyles = () => {
        ...
        if (s === 'in progress' || s === 'active') return "bg-blue-100 text-blue-700 border-blue-200";
        if (s === 'closed' || s === 'completed') return "bg-slate-100 text-slate-700 border-slate-200";
        if (s === 'pending') return "bg-amber-100 text-amber-700 border-amber-200";
        if (s === 'rejected') return "bg-red-100 text-red-700 border-red-200";
        if (s === 'applied') return "bg-indigo-100 text-indigo-700 border-indigo-200";
    ...
    return (
        <div
            onClick={onClick}
            className="group relative bg-white rounded-[24px] border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-500 hover:-translate-y-1.5 overflow-hidden flex flex-col h-full mx-auto w-full max-w-[350px] cursor-pointer"
        >
            {/* Status Badge */}
            ...
            <div className="p-5 flex flex-col flex-grow text-center sm:text-left">
                ...
                <div className="flex flex-wrap justify-center sm:justify-start gap-1.5 mb-5">
                    {tags && tags.slice(0, 3).map((tag, idx) => (
                        <span
                            key={idx}
                            className="px-2.5 py-1 bg-[#EFF6FF] text-[#2563EB] text-[9px] font-bold uppercase tracking-wider rounded-full border border-blue-100"
                        >
                            {tag}
                        </span>
                    ))}
