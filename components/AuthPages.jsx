    ...
    <Card className="w-full max-w-[350px] border-none shadow-2xl rounded-[1.5rem] bg-white pt-6 pb-5 px-4 z-10 transition-all duration-300 hover:shadow-[0_20px_50px_rgba(0,0,0,0.2)]">
      <CardHeader className="space-y-4 text-center pb-2 px-2">
        ...
        <div className="space-y-1">
          <CardTitle className="text-xl font-semibold text-slate-800 tracking-tight">Log In</CardTitle>
      ...
      <CardContent className="px-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                <Label htmlFor="email" className="text-xs font-semibold text-slate-700">Email <span className="text-red-500">*</span></Label>
              </div>
            </div>
          ...
          <div className="text-center pt-1">
            <p className="text-slate-500 text-sm font-medium">
              ...
              <button type="button" onClick={() => setCurrentView("register")} className="font-bold text-[#1e4db7] hover:text-[#1a43a1] underline decoration-2 underline-offset-4">
                Signup
              </button>
            </p>
          </div>
        ...
        <button onClick={() => setCurrentView("landing")} className="mt-6 flex w-full items-center justify-center gap-2 text-xs font-semibold text-slate-400 hover:text-slate-600 transition-colors uppercase tracking-widest">
  ...
  return (<div className="flex min-h-screen items-center justify-center p-4 relative overflow-hidden bg-[#1e4db7] py-12">
    ...
    <Card className="w-full max-w-[350px] border-none shadow-2xl rounded-[1.5rem] bg-white pt-6 pb-5 px-4 z-10 transition-all duration-300 hover:shadow-[0_20px_50px_rgba(0,0,0,0.2)]">
      <CardHeader className="space-y-4 text-center pb-2 px-2">
        ...
        <div className="space-y-1">
          <CardTitle className="text-xl font-semibold text-slate-800 tracking-tight">Create account</CardTitle>
          <CardDescription className="text-slate-500 text-xs font-medium leading-relaxed max-w-[95%] mx-auto">
      ...
      <CardContent className="px-6">
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {/* Role Selection */}
          <div className="space-y-1">
            <Label className="text-xs font-semibold text-slate-700">I want to</Label>
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => updateField("role", "client")} className={`flex flex-col items-center gap-1.5 rounded-xl border-2 p-2 transition-all ${formData.role === "client"
                ...
                <span className={`text-[10px] font-semibold ${formData.role === "client" ? "text-[#1e4db7]" : "text-slate-500"}`}>Hire Talent</span>
              </button>
              <button type="button" onClick={() => updateField("role", "freelancer")} className={`flex flex-col items-center gap-1.5 rounded-xl border-2 p-2 transition-all ${formData.role === "freelancer"
                ...
                <span className={`text-[10px] font-semibold ${formData.role === "freelancer" ? "text-[#1e4db7]" : "text-slate-500"}`}>Find Work</span>
              </button>
            </div>
          </div>
          ...
          {formData.role === "freelancer" && (<div className="space-y-1 animate-in fade-in slide-in-from-top-2">
            <Label htmlFor="skills" className="text-xs font-semibold text-slate-700">Skills (comma separated)</Label>
          ...
          <div className="text-center pt-1">
            <p className="text-slate-500 text-sm font-medium">
              ...
              <button type="button" onClick={() => setCurrentView("login")} className="font-bold text-[#1e4db7] hover:text-[#1a43a1] underline decoration-2 underline-offset-4">
                Sign in
              </button>
            </p>
          </div>
        ...
        <button onClick={() => setCurrentView("landing")} className="mt-4 flex w-full items-center justify-center gap-2 text-[10px] font-semibold text-slate-400 hover:text-slate-600 transition-colors uppercase tracking-widest">
