...
export default function FreelancerDashboard() {
    ...
    return (<div className="space-y-6">
        ...
        <CardContent className="p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              {user.skills?.slice(0, 4).map((skill) => (<Badge key={skill} variant="secondary" className="bg-secondary text-secondary-foreground">
                  {skill}
                </Badge>))}
            </div>
          </div>
      ...
      {/* Available Projects */}
      {availableProjects.length > 0 && (<Card className="border-border border-l-4 border-l-primary">
          <CardHeader className="flex flex-row items-center justify-between">
          ...
          <CardContent>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {availableProjects.slice(0, 3).map((project) => (<ProjectCard key={project._id} project={{
                    ...project,
                    tags: project.skills,
                    budget: `₹${project.budget.toLocaleString()}`,
                    milestones: project.milestones.length,
