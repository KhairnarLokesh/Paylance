...
export default function LandingPage() {
    ...
    return (<div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-blue-100">
      ...
      <header className="sticky top-0 z-50 bg-slate-950/80 backdrop-blur-md border-b border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="hidden md:flex items-center gap-6">
              <div className="flex items-center gap-6 text-sm font-medium text-slate-300 mr-2">
                <button onClick={() => document.getElementById('demo-projects')?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-blue-500 transition-colors">
                  Explore
                </button>
                <button onClick={() => document.getElementById('why-paylance')?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-blue-500 transition-colors">
                  Why Paylance
                </button>
              ...
              {user && (<button onClick={() => setCurrentView(user.role === 'client' ? 'client-dashboard' : 'freelancer-dashboard')} className="flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white rounded-full font-bold hover:bg-blue-700 shadow-md shadow-blue-100 transition-all active:scale-95">
                  <LayoutDashboard className="h-4 w-4"/>
                  Dashboard
                </button>)}

              {user?.role === 'client' && (<button onClick={() => setCurrentView('create-project')} className="flex items-center gap-1.5 px-4 py-2 bg-blue-50 text-blue-700 rounded-full font-bold hover:bg-blue-100 transition-all border border-blue-200">
              ...
              {user ? (<div className="flex items-center gap-4">
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon" className="relative h-10 w-10 rounded-full hover:bg-slate-800 transition-colors">
                        <Bell className="h-5 w-5 text-slate-300"/>
                        {unreadCount > 0 && (<span className="absolute top-1 right-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-600 text-[10px] font-bold text-white border-2 border-slate-900">
                            {unreadCount}
                          </span>)}
                      </Button>
                    </DropdownMenuTrigger>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full hover:bg-slate-800 transition-colors border border-slate-700 overflow-hidden">
                        {user.profile_image ? (<img src={user.profile_image} alt={user.name} className="h-full w-full object-cover"/>) : (<User className="h-5 w-5 text-slate-300"/>)}
                      </Button>
                    </DropdownMenuTrigger>
            ...
            {/* Mobile Menu Button */}
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="flex md:hidden p-2 text-slate-300 hover:bg-slate-800 rounded-lg transition-colors ml-2">
        ...
        {/* Mobile menu content */}
        {isMenuOpen && (<div className="md:hidden border-t border-slate-800 bg-slate-950 absolute top-20 left-0 w-full shadow-xl z-[60] py-6 px-6 space-y-4 animate-in slide-in-from-top-2 duration-200">
            <button onClick={() => {
                document.getElementById('demo-projects')?.scrollIntoView({ behavior: 'smooth' });
                setIsMenuOpen(false);
            }} className="block w-full text-left text-base font-semibold text-slate-300 hover:text-blue-500 py-2">
              Explore
            </button>
            <button onClick={() => {
            ...
            {user ? (<div className="space-y-4 pt-2">
                ...
                <button onClick={() => {
                    ...
                    setIsMenuOpen(false);
                }} className="flex items-center gap-3 w-full text-left text-base font-bold text-slate-300 py-2">
              ...
              </div>) : (<Button className="w-full h-12 bg-blue-700 hover:bg-blue-800 text-white rounded-xl font-bold shadow-lg shadow-blue-100 transition-all active:scale-95" onClick={() => {
      ...
      <section id="hero" className="relative pt-16 pb-24 bg-slate-950 text-white overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="space-y-10 z-10">
              <div className="relative h-[320px] md:h-[420px]">
                {slides.map((slide, index) => (<div key={index} className={`absolute inset-0 transition-all duration-1000 ease-in-out ${index === currentSlide ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"}`}>
              ...
              <div>
                <Button size="lg" className="bg-blue-600 hover:bg-blue-700 text-white rounded-full px-8 h-12 text-base font-medium group" onClick={() => setCurrentView('register')}>
            ...
            <div className="relative h-[600px] w-full rounded-2xl overflow-hidden bg-slate-900 border border-slate-800 shadow-sm transition-all hover:shadow-md">
              {slides.map((slide, index) => (<div key={index} className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${index === currentSlide ? "opacity-100 z-10" : "opacity-0 z-0"}`}>
          ...
          <div className="flex justify-center gap-2 mt-12 w-full">
            {slides.map((_, index) => (<button key={index} onClick={() => setCurrentSlide(index)} className={`h-2 rounded-full transition-all ${index === currentSlide
                ? "w-2 bg-blue-600"
                : "w-2 bg-slate-800 hover:bg-slate-700"}`} aria-label={`Go to slide ${index + 1}`}></button>))}
          </div>
        </div>
      </section>
      ...
      {/* Features Section */}
      <section id="why-paylance" className="py-24 bg-slate-950 text-white border-t border-slate-800 relative">
        <div className="absolute inset-0 bg-[linear-gradient(to_right,#4f4f4f2e_1px,transparent_1px),linear-gradient(to_bottom,#4f4f4f2e_1px,transparent_1px)] bg-[size:14px_24px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] opacity-20 pointer-events-none"/>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-white">Why Paylance?</h2>
            <p className="text-slate-400 mt-4 max-w-2xl mx-auto">Everything you need to succeed in the freelance economy.</p>
          </div>
        ...
        ].map((item, i) => (<ScrollAnimation key={i} delay={i * 100} className="bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-800 hover:border-slate-700 hover:shadow-md transition-all hover:-translate-y-1 group">
      ...
      <section id="demo-projects" className="py-24 bg-slate-950 text-white overflow-hidden relative">
        <div className="absolute inset-0 bg-[linear-gradient(to_right,#4f4f4f2e_1px,transparent_1px),linear-gradient(to_bottom,#4f4f4f2e_1px,transparent_1px)] bg-[size:14px_24px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] opacity-20 pointer-events-none"/>

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col mb-12">
            <h2 className="text-4xl font-extrabold text-white tracking-tight mb-4">
              Explore Our <span className="text-blue-500">Demo Projects</span>
            </h2>
          ...
          <div className="relative group/slider">
            ...
            <div className="flex justify-center gap-1.5 mt-4">
              <div className="h-1.5 w-12 rounded-full bg-blue-900/50 relative overflow-hidden">
