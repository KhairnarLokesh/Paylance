...
export default function ProfilePage() {
    ...
    return (<div className="max-w-6xl mx-auto pb-20">
            ...
            <div className="flex items-center gap-2 text-sm text-slate-500 mb-6 px-4">
                ...
                {user?.role === 'client' && (<button onClick={() => setCurrentView('create-project')} className="ml-auto flex items-center gap-1.5 px-4 py-2 bg-blue-50 text-blue-700 rounded-full font-bold hover:bg-blue-100 transition-all border border-blue-200">
                            ...
                            <div>
                                <div className="flex items-center gap-3 group">
                                    {isEditingName ? (<div className="flex items-center gap-2">
                                            <Input value={tempName} onChange={(e) => setTempName(e.target.value)} className="text-3xl font-extrabold h-14 w-80 rounded-xl" autoFocus/>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-16 mt-16 border-t border-slate-50 pt-16">
                        ...
                        <div className="space-y-6">
                            <div className="flex justify-between items-center group">
                                ...
                                <button className="flex items-center gap-1.5 text-xs font-bold text-blue-600 hover:text-blue-700 transition-colors uppercase tracking-wider" onClick={() => setIsAddingSkill(true)}>
                                    <Plus className="h-4 w-4"/>
                                    Add Skill
                                </button>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-3">
                                {user?.skills && user.skills.length > 0 ? (user.skills.map((skill, idx) => (<div key={idx} className="group flex items-center gap-2 px-4 py-2 bg-slate-50 text-slate-700 font-bold text-sm rounded-xl border border-slate-100 hover:bg-white hover:border-blue-200 transition-all cursor-default">
                                            {skill}
                        ...
                        {user?.role === 'client' && (<div className="space-y-6">
                                <div className="flex justify-between items-center">
                                    ...
                                    <button className="text-xs font-bold text-blue-600 hover:underline" onClick={() => setCurrentView('my-projects')}>
                                        Manage All
                                    </button>
                                </div>
                                <div className="space-y-3">
                                    {myProjects.length > 0 ? (myProjects.slice(0, 3).map((proj) => (<div key={proj._id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex justify-between items-center group/proj hover:bg-white hover:border-blue-100 transition-all">
                                            ...
                                            <button onClick={() => setCurrentView('create-project')} className="text-xs font-bold text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-200">
