...
export default function MessagesPage() {
  ...
  const handleSend = async (e) => {
    ...
    const receiverId =
      user.role === "client" ? currentChat.assignedTo : currentChat.clientId;
  ...
  const selectChat = (project) => {
    const otherUserId =
      user.role === "client" ? project.assignedTo : project.clientId;
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] gap-4">
      {/* Chat List */}
      <Card
        className={`border-border ${selectedChat ? "hidden lg:flex" : "flex"
            ...
            <div className="divide-y divide-border">
              {chatProjects.map((project) => {
                ...
                return (
                  <button
                    key={project._id}
                    onClick={() => selectChat(project)}
                    className={`flex w-full items-start gap-3 p-4 text-left transition-colors hover:bg-muted ${isSelected ? "bg-muted" : ""
                      }`}
                  >
      ...
      {/* Chat Window */}
      <Card
        className={`flex-1 border-border ${selectedChat ? "flex" : "hidden lg:flex"
          } flex-col`}
      >
        {!selectedChat ? (
